                ...
                for y in self.apply(x, **self.apply_kwargs):
                    ...
                    if self.add_to_session:
                        ...
                        if len(insert_batch) >= INSERT_BATCH_SIZE:
                            # Sadly bulk_save_objects doesn't work here if we have to wait on inserts to map relationships. 
                            # self.session.bulk_save_objects(insert_batch, return_defaults=True)
                            self.session.add_all(insert_batch)
                            self.session.flush()
                            insert_batch.clear()
